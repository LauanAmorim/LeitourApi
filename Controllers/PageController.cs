...
namespace LeitourApi.Controllers
{
    ...
    public class PageController : ControllerBase
    {
        ...
        public readonly IPageService uow.PageRepository;
        ...
        {
            List<Page>? pages = await uow.PageRepository.GetAllPages();
            return (pages == null) ? message.NotFound(): pages;
        }

        [HttpGet("name/{namePage}")]
        public async Task<ActionResult<IEnumerable<Page>>> GetPageByName(string namePage)
        {
            List<Page>? pages = await uow.PageRepository.GetPagesByName(namePage);
            return (pages == null) ? message.NotFound(): pages;
        }
        ...
        {
            Page? page = await uow.PageRepository.GetById(PageId);
            return (page == null) ? _msgService.MsgPagemessage.NotFound() : Ok(page);
        }
        ...
        public async Task<ActionResult<dynamic>> PostPage([FromHeader] string token, [FromBody] Page Page)
        {
            ...
            Page = await uow.PageRepository.CreatePage(Page,id);
        ...
        public async Task<IActionResult> PutPage(int PageId, [FromHeader] string token, [FromBody] Page NewPage)
        {
            ...
            var page = await uow.PageRepository.GetById(PageId);

            if (page == null)
                return _msgService.MsgPagemessage.NotFound();

            var adminUser = await uow.PageRepository.VerifyAdmin(PageId,id);

            if (adminUser)
                return BadRequest("Você não é da admistração dessa página");

            uow.PageRepository.UpdatePage(NewPage);
        ...
        public async Task<IActionResult> FollowPage([FromHeader] string token, int pageId)
        {
            ...
            bool alreadyFollowing = await uow.PageRepository.VerifyFollowing(id, pageId);

            if(alreadyFollowing)
                return BadRequest("Você já está seguindo esta página");
            
            await uow.PageRepository.FollowPage(id, pageId);
        ...
        public async Task<IActionResult> UnfollowPage([FromHeader] string token, int pageId)
        {
            ...
            uow.PageRepository.UnfollowPage(id, pageId);
        ...
        public async Task<ActionResult<IEnumerable<Page?>>> FollowingPage([FromHeader] string token)
        {
            ...
            if (User == null)
                return _msgService.MsgUsermessage.NotFound();

            List<Page?> followingPages = await uow.PageRepository.GetPageList(id);

            if (followingPages == null)
                return message.NotFound()($"Você não segue nenhuma página");
        ...
        public async Task<ActionResult<IEnumerable<User>>> PageFollowers(int PageId)
        {
            ...
            var pageFollowers = await uow.PageRepository.GetPageFollowers(PageId);

            if (pageFollowers == null)
                return message.NotFound()($"Essa pagina não tem seguidores");
